...
| **Tracks the dates that will move your stocks** | Veda keeps per-company calendars for earnings, AGMs, ex-dividends, and splits, plus a global macro calendar for FOMC and pasted CPI/RBI dates. Past events automatically move out of upcoming, so the next decision point stays visible. | *"What earnings calls are coming up in my portfolio next month?"* <br><br> *"When is HDFC Bank's next AGM and ex-dividend?"* <br><br> *"Show me the FOMC meeting dates for the rest of the year."* |
| **Watches who's buying and selling on the inside** | Veda pulls insider trades straight from SEC Form 4 (US) and NSE PIT (India), filtering ESOPs, ADR/GDR routing, and small lots so only meaningful open-market activity surfaces. It also tracks promoter and institutional shareholding patterns quarter by quarter, so you can see when a CEO sells $50M, when promoter holding shifts, or when an Indian small-cap's promoter pledging changes. | *"Has any insider sold Microsoft stock in the last 90 days?"* <br><br> *"Show me promoter and FII holding trends for HDFC Bank over the last two years."* <br><br> *"Did any NTPC executive trade around the last earnings call?"* |
| **Narrows a whole sector down to a short list worth reading** | Sizing up a sector means wading through twenty or more companies to find the few worth real work. You give Veda the list of names in that sector and your own pass-or-fail rules — say, a minimum return on capital and a maximum level of debt. Veda keeps the names that pass, drops the ones that fail (and tells you which rule each one broke), and points out any name it is missing numbers for. It is a first-pass filter, not a buy list: it does not rank the survivors or pick a winner, and the final read is yours. This one feature reads its numbers from the companion Veda-research project, so you need that installed alongside this one. | *"Refresh the airlines cohort, then screen it."* <br><br> *"Screen the semiconductor sector and show which names pass my rules."* <br><br> *"Which names did my debt and return-on-capital filters drop?"* |
...
**v0.6.0 — early preview.** Walking skeleton with all eleven frameworks shipped (Lynch is the reference implementation; Buffett, Munger, Fisher, Druckenmiller, Marks, Klarman, Thorp, Dalio, Templeton, and Taleb ship alongside it as first-pass distillations — deeper per-framework citations are in flight), nine shipped subagents (devil's advocate, portfolio parser, company KB builder, fundamentals fetcher, base-rate researcher, news researcher, disclosure-fetcher, calendar-tracker, ownership-tracker), one worked example, a profile template with a worked example (aggressive), the [docs/](docs/) hub for users and contributors, and install paths for every major assistant. More worked examples and more broker importers are in flight.
