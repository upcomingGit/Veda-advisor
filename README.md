...
[![version](https://img.shields.io/badge/version-0.4.0-blue)](CHANGELOG.md) [![license](https://img.shields.io/badge/license-MIT-lightgrey)](LICENSE)
...
| **Tracks your companies for you** | You don't have to read every quarterly result, every news headline, every insider trade. Veda pulls news from major business publications first, then per-ticker Google News when coverage is thin, runs each item through a six-layer filter (catches stock-watching aggregators, Cramer commentary, "Is X a buy?" listicles, same-event duplicates across publishers), and grades the survivors against *your specific thesis* for the position — strengthens it, weakens it, or doesn't move it. You only see the events that matter to your reasons for owning the stock. | *"What has changed for Asian Paints in the last 30 days that I should care about?"* <br><br> *"Have any of my holdings had insider selling in the last quarter?"* <br><br> *"Summarise the key takeaways from HDFC Bank's latest earnings call."* |
| **Pulls primary regulator filings, not just news about them** | News articles are someone else's interpretation of what the company filed. Veda goes straight to the source: SEC EDGAR for 8-Ks (US), BSE and NSE corporate-announcement feeds (India). Trading-window closures, investor-complaint statements, ESOP grants and other routine SEBI compliance noise are filtered out automatically (12 patterns ported from a real production system, all with zero analytical value). What you see is the small set of filings that actually move the thesis — leadership changes, results, material agreements, board-meeting outcomes, dated future events that should land on your calendar. | *"Show me the latest 8-K filings for Microsoft."* <br><br> *"What corporate announcements has Reliance made in the last 30 days?"* <br><br> *"Has NTPC filed anything material since I last checked?"* |
| **Tracks the dates that will move your stocks** | The biggest single-day moves in a position usually arrive on dates you could have circled in advance — the next earnings call, the AGM, the ex-dividend cut-off, the FOMC meeting, the next CPI print. Veda keeps a calendar for every position you own (earnings dates, AGMs, ex-dividends, splits — pulled from yfinance for US tickers and Screener.in for India) plus a global macro calendar (FOMC dates auto-fetched from federalreserve.gov; US CPI, RBI MPC, and India CPI release schedules accepted directly from you in v1, with auto-fetch deferred until upstream feeds stabilise). Past dates are moved out of "upcoming" automatically on every refresh, so what's in front of you is always still ahead. | *"What earnings calls are coming up in my portfolio next month?"* <br><br> *"When is HDFC Bank's next AGM and ex-dividend?"* <br><br> *"Show me the FOMC meeting dates for the rest of the year."* |
...
**v0.4.0 — early preview.** Walking skeleton with all eleven frameworks shipped (Lynch is the reference implementation; Buffett, Munger, Fisher, Druckenmiller, Marks, Klarman, Thorp, Dalio, Templeton, and Taleb ship alongside it as first-pass distillations — deeper per-framework citations are in flight), eight shipped subagents (devil's advocate, portfolio parser, company KB builder, fundamentals fetcher, base-rate researcher, news researcher, disclosure-fetcher, calendar-tracker), one worked example, a profile template with two worked examples (aggressive and novice), the [docs/](docs/) hub for users and contributors, and install paths for every major assistant. More worked examples and more broker importers are in flight.
