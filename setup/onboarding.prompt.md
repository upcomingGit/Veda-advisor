...
| `broker.primary` | First time the user says *"refresh from Zerodha"*, *"pull from broker"*, *"refresh holdings"*, or similar live-pull request | *"Which broker do you use for this portfolio? (zerodha / other)"* Write `zerodha`, `other`, or `none` to `profile.md > broker.primary`. If zerodha → proceed with live pull per pattern 4 (internal/assets-update-procedures.md). If other/none → fall back to paste. |
