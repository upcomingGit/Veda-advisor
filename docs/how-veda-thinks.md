...
- **Recent regulator filings** via the `disclosure-fetcher` subagent — pulls unscheduled material announcements from primary regulator feeds: SEC EDGAR 8-K (US), BSE and NSE corporate-announcement APIs (India). Drops 12 objectively-routine SEBI compliance patterns at the fetch stage (trading-window closures, investor complaints, ESOP grants — all with documented zero analytical value). Refuses user-pasted articles. When a filing announces a future scheduled event (board meeting, AGM date), the date is routed to the position's calendar. Caches into `holdings/<slug>/disclosures.md`; re-runs after 24 hours.
- **Scheduled corporate and macro events** via the `calendar-tracker` subagent — maintains per-position calendars (next earnings, AGM, ex-dividend, splits — yfinance for US, Screener.in for India) and a global macro calendar (FOMC dates from federalreserve.gov; US CPI / RBI MPC / India CPI release schedules deferred to v2 and accepted via user-pasted dates in v1). Strict dedup with `disclosure-fetcher` and `earnings-grader` rows — preserves anything an authoritative source has already written. Past events move from `upcoming:` to `past:` on every invocation. Caches for 30 days.
- **Insider/promoter trades and shareholding pattern** via the `ownership-tracker` subagent — pulls insider activity from primary exchange feeds: SEC EDGAR Form 4 via the submissions JSON (US, with per-filing index.json scraping for the raw ownership XML and lot aggregation across same-day same-direction transactions), NSE PIT API (India, with the StockClarity-ported 5-filter pipeline that drops ESOPs, ADR/GDR routing, off-market transfers, and lots below ₹1 Cr buy / ₹5 Cr sell). Quarterly shareholding splits via the NSE corp-shp master endpoint (promoter and public aggregates with up to 8 quarters of history; FII/DII split lives inside per-filing XBRL and is parked for v2) with a BSE shareholding-pattern fallback. Stable transaction IDs (US `<accession>-<insider-slug>-<P|S>`; India `<date>-NSE-<acqName-slug>-<B|S>-<shares>`) so re-runs reliably dedup against the cached `insiders.yaml`. Refuses user-pasted insider transactions or shareholding text in v1; the narrow `pasted_pledging` channel is US-only and accepts a single validated number. Split-TTL gating — `insiders.yaml` re-runs after 7 days (event-driven), `shareholding.yaml` after 30 days (quarterly snapshot); both bypass the cache on Stage 9a buy/sell hold-checks. Hard 5-operation web cap; 50 transactions per `insiders.yaml`, 8 quarters per `shareholding.yaml > history`. India promoter pledging is parked for v2 (`pledging_status: not_fetched_v1` — orchestrator treats absence as "unknown", not "0%").
