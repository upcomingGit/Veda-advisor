...
- **Leverage implementation.** Bridgewater's All Weather uses leverage on long-duration bonds to equalize risk contribution; most retail users cannot and should not replicate this. Veda applies the conceptual version only, and refuses leverage when the client's `instruments.margin` is false.
