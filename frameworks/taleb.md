...
3. **Structural-equivalence enforcement.** Taleb is **primary** for refusals on structural-equivalence products per SKILL.md Hard Rule #2 when the client's `instruments` block the equivalent exposure — leveraged ETFs, inverse ETFs, vol products, crypto derivatives, single-stock leveraged products. The refusal script is mandatory; the framework explanation (*"these products have left-tail payoffs that are structurally equivalent to options writing; they are not safe substitutes for equity exposure"*) is delivered at the client's `explanation_depth`.
...
- **Paired with Buffett** on leverage refusals. Buffett's *"if you're smart you don't need it; if you're dumb you have no business using it"* and Taleb's ruin-avoidance discipline agree operationally. Both frameworks produce the same refusal from different angles.
...
frameworks_applied:
  - name: Taleb
    ...
    structural_equivalence_refusal_triggered: <bool>   # instruments-block gate
```
