...
> **REDUNDANT — BACKUP, NOT THE LIVE SKILL.** This is the pre-re-center Veda pipeline,
> kept when the advisor was re-centered on five jobs (see the live `SKILL.md`). It is
> retained only as the **uncovered-name analysis engine** — the data-completeness gate,
> the per-name fetchers (news, fundamentals, disclosures, calendar, ownership), the
> base-rate outside view, framework routing, framework application, and the
> devil's-advocate synthesis (old Stages 3–7) — that the live SKILL falls back to for a
> name the research house does not cover. Do not follow this file as the primary
> pipeline.
>
> **Cut-step note.** The subagent definitions this pipeline invokes are now quarantined
> alongside it in `redundant/agents/`, and their fetcher scripts in `redundant/scripts/`.
> Path references below that still read `internal/agents/...` or `scripts/fetch_...` are
> historical — the files moved to `redundant/` in the cut step.
