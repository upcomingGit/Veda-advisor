...
**Total: 10 subagents (4 shipped, 6 to build).**
...
| 2 | `portfolio-parser` | **Shipped** | (no files — returns structured YAML to orchestrator) | [`internal/agents/portfolio-parser.md`](agents/portfolio-parser.md) |
| 3 | `base-rate-researcher` | Planned | (no files — returns stats to orchestrator) | `internal/agents/base-rate-researcher.md` (to create) |
| 4 | `company-kb-builder` | **Shipped** | `kb.md`, `thesis.md` (draft or proposed updates), `governance.md`, `risks.md`, `_meta.yaml` archetype | [`internal/agents/company-kb-builder.md`](agents/company-kb-builder.md) |
...
| 7 | `earnings-grader` | Planned | `earnings/<quarter>.md`, `assumptions.yaml`, `calendar.yaml` (transcript-pending events) | `internal/agents/earnings-grader.md` (to create) |
| 8 | `ownership-tracker` | Planned | `insiders.yaml`, `shareholding.yaml` | `internal/agents/ownership-tracker.md` (to create) |
| 9 | `calendar-tracker` | Planned | `calendar.yaml`, `disclosures.md`, `global_calendar.yaml` (root) | `internal/agents/calendar-tracker.md` (to create) |
| 10 | `indicators-researcher` | Planned | `indicators.yaml` | `internal/agents/indicators-researcher.md` (to create) |
...
- **`company-kb-builder`** — one-shot per ticker. Produces qualitative narrative research: business-model summary, competitor list, macro profile, governance/red-flag assessment, position-risk register, and a first-draft investment thesis with archetype classification (GROWTH / INCOME_VALUE / TURNAROUND / CYCLICAL). Source material: filings, investor letters, web research (`tools: Read, WebFetch`). Cache-skips if `kb.md` is < 365 days old unless `force_refresh: true`. Thesis-reconciliation: if the user has edited `thesis.md`, appends `## Proposed updates` — never overwrites. Workspace must be pre-scaffolded by the orchestrator. **Invoked from Stage 1.5 when kb.md is a stub for a held position, and on explicit 'build kb for <ticker>' requests.** The thesis draft is a starting point — the user can (and should) edit it to reflect their own conviction. Input/output contract and regression-test anchors live in the definition file linked above.
- **`fundamentals-fetcher`** — refreshed quarterly (post-earnings) or on explicit request. Pulls structured financial data via the project's `scripts/fetch_fundamentals.py` adapter: **yfinance** for US equities, **Screener.in** for Indian equities. Writes `fundamentals.yaml` (12 quarters of P&L, cash flow, balance-sheet snapshots) and `valuation.yaml` (current vs. historical multiples with archetype-specific zone classification — PEG size-tiered for GROWTH, PE percentile + PEG override for INCOME_VALUE, EV/EBITDA for TURNAROUND/CYCLICAL with commodity-cyclical inversion, P/B override for banks/NBFCs). Tools: `Bash, Read, Write` — the fetch script is the only Bash invocation; the subagent itself does no scraping or arithmetic. The script computes zones; the subagent transcribes. Cache-skip: if the latest stored quarter equals the current calendar quarter, skip. Workspace must be pre-scaffolded by the orchestrator. Separate from `company-kb-builder` because data source, refresh cadence, and failure modes are all different — quant snapshots stale in quarters, narrative research stales in years. Input/output contract and 8 regression-test anchors live in the definition file linked above.
...
| `ownership-tracker` | `holdings/<instance>/` | `insiders.yaml`, `shareholding.yaml` | Quarterly or on significant change | BSE/NSE (India), SEC Form 4 (US) |
| `calendar-tracker` | `holdings/<instance>/` and workspace root | `calendar.yaml`, `disclosures.md`, `global_calendar.yaml` | Per event discovery | Company IR, exchange calendars, central-bank schedules |
| `indicators-researcher` | `holdings/<instance>/` | `indicators.yaml` | Varies by indicator | Industry reports, trade associations, government data |
