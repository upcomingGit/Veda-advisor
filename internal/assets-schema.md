...
size (with Veda's help); once set, it's mirrored into `dynamic.target_weights`
(above) so the rebalancer proposes the opening buy. `trigger` is the price or
event that would make you act.
