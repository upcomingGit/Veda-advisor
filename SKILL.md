...
3. **Narrate the load.** One line, naming the files loaded:
   ...
   **Handle the subagent's response.** The subagent returns a `company_kb_builder:` block with a `status` field:
   ...
   - `status: ok` → **verify writes before trusting the status block.** Read each path listed in `files_written` from disk. For each file: (a) confirm it exists, (b) confirm byte size > 200 (a stub `_(to be populated)_` is ~22 bytes; a real KB file is thousands), (c) confirm it does not contain only the literal `_(to be populated)_` placeholder. If any file fails verification, treat the response as `status: failed` (see below) regardless of the reported status — the subagent (or inline-bridge LLM) hallucinated the writes. This is a known failure mode on surfaces without true subagent isolation. On verification pass, reload `kb.md` and `thesis.md` from disk; use them for the current decision pipeline. Narrate: *"KB built for <ticker> (verified <N> files written). Proceeding with populated kb.md and thesis.md."*
   ...
   - `status: failed` (including verification-induced failure) → do not block the decision pipeline. On inline surfaces, retry the write **once** by re-reading the four file schemas from the subagent definition and writing the content directly via the editor tool. If the retry also fails verification, fall back to `assets.md` thesis column and general knowledge. Narrate: *"KB build failed for <ticker> (<warning>). Falling back to assets.md thesis."*
