...
3. **Narrate the load.** One line, naming the files loaded:
   ...
   **Delegation, where available.** When `kb.md` is a stub for a held position, on any host that supports isolated subagent execution — Claude Code, GitHub Copilot, Google Antigravity, or equivalent — delegate knowledge-base construction to the `company-kb-builder` subagent ([`internal/agents/company-kb-builder.md`](internal/agents/company-kb-builder.md)). The subagent writes `kb.md`, `thesis.md` (first draft), `governance.md`, and `risks.md` directly into `holdings/<slug>/` and returns a status block. The canonical definition lives in `internal/agents/`; host-facing copies at `.claude/agents/` are generated by [`scripts/sync_agents.py`](scripts/sync_agents.py) — do not edit them directly. See [internal/subagents.md](internal/subagents.md) for the full host-discovery table.

   Pass the input block defined in the subagent's contract: `ticker`, `instance_key`, `market`, `archetype` (from `_meta.yaml`), `force_refresh: false`, `kb_age_days: null` (on first build), `thesis_is_stub: true`, and any user-supplied `additional_context`. The subagent writes files directly; the orchestrator does not touch those files.

   **Handle the subagent's response.** The subagent returns a `company_kb_builder:` block with a `status` field:

   - `status: ok` → reload `kb.md` and `thesis.md` from disk; use them for the current decision pipeline. Narrate: *"KB built for <ticker>. Proceeding with populated kb.md and thesis.md."*
   - `status: partial` → reload what was written; note the gaps from `warnings` in the session context. Narrate: *"KB partially built for <ticker> — some sections are stubs. Proceeding with available content."*
   - `status: skipped` → kb is fresh (< 365 days old). Use the existing `kb.md` as-is. No narration needed beyond the normal load line.
   - `status: failed` → do not block the decision pipeline. Fall back to `assets.md` thesis column and general knowledge. Narrate: *"KB build failed for <ticker> (<warning>). Falling back to assets.md thesis."*
   ...
   On surfaces without subagent isolation, produce the same four files inline using the input/output contract from the subagent definition file. The current-session pipeline continues either way; isolation is lost in inline mode, but the structured file output is identical.
...
4. **Workspace missing for a held ticker — scaffold it.** If `assets.md` holds the ticker but `holdings/<slug>/` does not exist, scaffold the workspace per [internal/holdings-schema.md](internal/holdings-schema.md) § "Creation behavior" — determine archetype (inference table there; ask only when ambiguous), create `_meta.yaml` + stub `kb.md` + stub `thesis.md` + empty `decisions/`, and append the registry row. **Do not scaffold for tickers the user is merely considering** — scaffolding waits for a commit event (held position or a `buy` decision at Stage 9a). Evaluation questions on non-held tickers proceed without a workspace; the journal records the verdict. Narrate:
   ...
   **Then build the KB on the same turn.** A freshly-scaffolded workspace contains only stubs. For a held position, the user's question deserves better than the "assets.md thesis column and general knowledge" fallback. Apply the Step 3 delegation block now: invoke `company-kb-builder` with `thesis_is_stub: true` and `kb_age_days: null`, wait for the response, reload the populated files, and proceed to Stage 1.6 with real KB content. Subagent skip-paths (`status: failed`) fall back to the same general-knowledge mode — the decision pipeline does not block on KB build failures.
...
**Delegation, where available — fundamentals refresh.** When the question is a held position and requires fundamentals or valuation data (`when` problems, valuation questions, post-earnings reviews), check `holdings/<slug>/fundamentals.yaml` and `valuation.yaml`. If either is missing, or if `fundamentals.yaml`'s latest `period` is older than the current calendar quarter, on any host that supports isolated subagent execution — Claude Code, GitHub Copilot, Google Antigravity, or equivalent — delegate the refresh to the `fundamentals-fetcher` subagent ([`internal/agents/fundamentals-fetcher.md`](internal/agents/fundamentals-fetcher.md)). Pass the input block defined in the subagent's contract: `ticker`, `instance_key`, `market`, `archetype` (from `_meta.yaml`), `sector` and `sector_kind` (if known; `OTHER` and `null` are acceptable), `force_refresh: false`, `latest_stored_quarter` (parse from existing `fundamentals.yaml` or `null`).

**Handle the subagent's response.** The subagent returns a `fundamentals_fetcher:` block with a `status` field:

- `status: ok` → reload the two YAML files; use them for valuation/zone reasoning. Narrate: *"Fundamentals refreshed for <ticker>: <N> quarters; valuation zone <ZONE> on <metric>."*
- `status: partial` → reload what was written; the missing fields go into the `LOW-CONFIDENCE` flag for Stage 7. Narrate: *"Fundamentals partially refreshed for <ticker> — gaps: <warnings list>. Flagged for Stage 7."*
- `status: skipped` → latest quarter is current; use existing files as-is. No narration beyond a normal load line if it's relevant to the question.
- `status: failed` → do not block the pipeline. Either fetch via `scripts/fetch_quote.py` for the specific datum needed, or set `LOW-CONFIDENCE` and proceed. Narrate: *"Fundamentals refresh failed for <ticker> (<warning>). Falling back to per-datum fetches."*

On surfaces without subagent isolation, run `scripts/fetch_fundamentals.py` directly with the same parameters and apply the same status-handling logic inline. The output shape is identical either way.
...
Ten subagents are part of Veda's design — see [internal/subagents.md](internal/subagents.md) for the full inventory. Current status of the first three:
...
- **`company-kb-builder`** — **shipped**. Canonical definition at [`internal/agents/company-kb-builder.md`](internal/agents/company-kb-builder.md); host-facing copies at `.claude/agents/` are generated by [`scripts/sync_agents.py`](scripts/sync_agents.py). Invoked from Stage 1.5 when `kb.md` is a stub for a held position, and on explicit `build kb for <ticker>` requests. Sources: filings, investor letters, web research (`tools: Read, WebFetch`). Writes `kb.md`, `thesis.md` (first draft or `## Proposed updates`), `governance.md`, `risks.md`, and updates `_meta.yaml`. Cache-skips when `kb.md` is < 365 days old unless `force_refresh: true`. Contract and regression-test anchors live in the canonical file.
- **`fundamentals-fetcher`** — **shipped**. Canonical definition at [`internal/agents/fundamentals-fetcher.md`](internal/agents/fundamentals-fetcher.md). Invoked from Stage 3 when a held position's `fundamentals.yaml` or `valuation.yaml` is missing or stale (older than the current calendar quarter). Sources: yfinance (US), Screener.in (India) via `scripts/fetch_fundamentals.py` (`tools: Bash, Read, Write`). Writes 12 quarters of P&L, cash flow, and balance-sheet data plus a current valuation-zone snapshot per archetype — PEG size-tiered for GROWTH, PE-percentile-with-PEG-guard for INCOME_VALUE, EV/EBITDA for TURNAROUND/CYCLICAL (commodity-cyclical inverted), P/B for banks/NBFCs. The fetch script computes zones; the subagent transcribes. Contract and regression-test anchors live in the canonical file.
