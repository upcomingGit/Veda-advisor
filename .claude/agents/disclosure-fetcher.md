...
- **No edits to `calendar.yaml`.** When you extract a future scheduled event, you emit it in `proposed_calendar_entries`; the orchestrator writes it. This dovetails with the `calendar-tracker` subagent (both subagents emit calendar-shaped rows; orchestrator handles the actual append) and preserves the read-only filesystem boundary established by `news-researcher` and `base-rate-researcher`.
